...
Так же при установке [run](https://github.com/pt-main/run) tal устанавливается автоматически и доступен как `run tal ...`.

При первом запуске `tal update` создаст `.tal.pack` - файл с хешами файлов в текущей директории. Так же инициализировать проекты можно с помощью `tal init`
...
`tal run ...` поддерживает флаг `--deps="..."` который позволяет передать в скрипт зависимости для фнотации `-- #depends`. При отсутствии флага скрипт работает с `.tal.pack`.
...
Когда tal используется из  `run_cli` становится доступна дополнительная функция - `run_cli(args_string)`, которая напрямую вызывает run, и парсит аргументы из строки на входе.
