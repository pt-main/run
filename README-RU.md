...
**run** - это инструмент для управления скриптами, скриптования любых сценариев на встроенном lua-подобном языке с инкрементальностью, хранения скроптов в глобальном/локальном хранилище, полной независимостью от системы и платформы (работает везде куда компилируется go), и со встроенными способами дистрибуции скриптов, например через github. 
...
|---------|----------|--------|
| `-add <path> <name> [docs] [--force]` | Добавить скрипт (поддерживает `.py`, `.sh`, `.bat`, `.lua`) | `run -add script.py mypy` |
...
| `-r --tagged="..." --parallel` | Параллельный запуск скриптов с нужным тегом | `run -r --tagged="deploy;build" --parallel` |
...
| `-version` | Показать версию run и tal | `run -version` | 

`--no_color` – флаг отключает цветной вывод на протяжении сессии.
...
run -localmode       # вывести состояние
```
